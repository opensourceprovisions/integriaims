...
if ($delete) {
	...
	$sql = sprintf("DELETE FROM tcompany_field WHERE id = %d", $id);
	$res = process_sql($sql);
...
if (!empty($company_fields)) {
	foreach ($company_fields as $field) {
		$url_delete = "index.php?sec=customers&sec2=operation/companies/company_custom_fields&delete=1&id=".$field['id']."&name_delete=".$field["label"];
		$url_update = "index.php?sec=customers&sec2=operation/companies/company_custom_fields&id_field=".$field['id'];
		...
		if ($field["type"] == "linked") {
			$sql = "SELECT label FROM tcompany_field WHERE type = 'linked' AND id =".$field["parent"];
