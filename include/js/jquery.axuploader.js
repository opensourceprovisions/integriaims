		...
		init : function(options)
		{
    	    ...
    	    return this.each(function() 
    	    {
			   ...
			   // isAjaxUpload=false;
				...
				{
					// HACK TO RESTRICT MULTIUPLOADS
					// If one file is loaded, we replace it
					if(totalFiles > 0) {
						$('.ax-remove').trigger('click');
					}
					...
					var rm=$('<input class="ax-remove" type="button" value="Remove" />').css('display','none').bind('click',function(){
						fileCount--;
						totalFiles--;
						uploadAll.attr('disabled',fileCount==0);
					...
					var up=$('<input type="button" value="Upload" class="ax-upload" style="display:none;" />').bind('click',function(){ this.disabled=true; });
					
					var tr=$('<tr />').appendTo(t);
					var td_n=$('<td class="ax-file-name" title="'+n+'" />').appendTo(tr).html(n).css({'font-weight':'bold'});
					var td_s=$('<td class="ax-size-td" />').appendTo(tr).html(s);
					var td_p=$('<td class="ax-progress-td" />').appendTo(tr).css({'width':'100px','border':'solid 1px black','padding':'0px 0px 0px 0px','text-align':'center'});
					var div_p=$('<div  class="ax-progress-div" />').css({'width':'0px'}).html('0%').appendTo(td_p);
